...
void pid_p_only(void) {
  ...
  pid_controller.in = s_in;
  pid_controller.out = out;

  // Try proportinal with different values
  for(in = 0; in < it; in++) {
    s_in.write(in);
    TEST_ASSERT_EQUAL(in, s_in.read());
    pid_controller.simulate();
    TEST_ASSERT_EQUAL(in, s_in.read());
    TEST_ASSERT_EQUAL(in, pid_controller.in.read());
    TEST_ASSERT_EQUAL(in * gain, pid_controller.out.read());
  }
}
...
void pid_i_only(void) {
  ...
  for(in = 0; in < it; in++) {
    ...
    pid_controller.simulate();
    acc += in;
    TEST_ASSERT_EQUAL(acc * gain, pid_controller.out.read());
  }
}
...
void pid_i_only_limit(void) {
  ...
  for(in = 0; in < it; in++) {
    ...
    if (acc <= upper) {
      ...
      TEST_ASSERT_EQUAL(upper * gain, pid_controller.out.read());
    }
  }
...
void pid_d_only(void) {
  ...
  for(in = inc; in < it; in+= inc) {
    s_in =  in;
    pid_controller.simulate();
    TEST_ASSERT_EQUAL(inc * gain, pid_controller.out.read());
  }
...
void pid_all(void) {
  ...
  i = 0;
  d = 0;
  TEST_ASSERT_EQUAL(p + i + d, pid_controller.out.read());
